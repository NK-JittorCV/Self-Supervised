...
import models.vision_transformer_sere as vision_transformer
from models.vision_transformer_sere import iBOTHead, MultiCropWrapper
...
def train_sere(args):
    ...
    # ============ preparing data ... ============
    transform = DataAugmentationSERE(
    ...
    logger.info("Actual lr: %.2e" % (args.lr * (eff_batch_size) / 256.))
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_one_epoch(student, teacher, ibot_loss,
...
def train_one_epoch(student, teacher, ibot_loss, data_loader,
    ...
    losses_patch = utils.AverageMeter()
    losses_cls = utils.AverageMeter()
    losses_sere_pixel = utils.AverageMeter()
    losses_sere_channel = utils.AverageMeter()
    ...
    for it_current_epoch, (images, labels, masks) in enumerate(data_loader):
        ...
        cls_patch_loss = all_loss.pop('loss')

        loss = cls_patch_loss * (4 - args.alpha - args.beta) / 2.0 + loss_sere_pixel * args.alpha + loss_sere_channel * args.beta
        ...
        losses_patch.update(all_loss['patch'].item(), images[0].size(0))
        losses_cls.update(all_loss['cls'].item(), images[0].size(0))
        losses_sere_pixel.update(loss_sere_pixel.item(), images[0].size(0))
        losses_sere_channel.update(loss_sere_channel.item(), images[0].size(0))
        accs.update(acc)
        ...
        if jt.rank == 0 and it % 50 == 0:
            logger.info(
                ...
                "Patch {losses_patch.val:.4f} ({losses_patch.avg:.4f})\t"
                "Cls {losses_cls.val:.4f} ({losses_cls.avg:.4f})\t"
                "SERE_pixel {losses_sere_pixel.val:.4f} ({losses_sere_pixel.avg:.4f})\t"
                "SERE_channel {losses_sere_channel.val:.4f} ({losses_sere_channel.avg:.4f})\t"
                ...
                "Lr: {lr:.6f}\t Wd: {wd:.3f}".format(
                    ...
                    data_time=data_time,
                    losses=losses,
                    losses_patch=losses_patch,
                    losses_cls=losses_cls,
                    losses_sere_pixel=losses_sere_pixel,
                    losses_sere_channel=losses_sere_channel,
                    accs=accs, 
...
class iBOTLoss(nn.Module):
    ...
    def execute(self, student_output, teacher_output, student_local_cls, student_mask, epoch, it):
        """
        ...
        if student_local_cls is not None:
            student_cls = jt.concat([student_cls, student_local_cls])
        ...
        for q in range(len(teacher_cls_c)):
            for v in range(len(student_cls_c)):
                if v == q:
                    loss2 = jt.sum(-teacher_patch_c[q] * jt.nn.log_softmax(student_patch_c[v], dim=-1), dim=-1)
                    mask = student_mask[v].flatten(-2, -1)
                    loss2 = jt.sum(loss2 * mask.float(), dim=-1) / mask.sum(dim=-1).clamp(min_v=1.0)
                    total_loss2 += loss2.mean()
                    n_loss_terms2 += 1
                else:
                    loss1 = jt.sum(-teacher_cls_c[q] * jt.nn.log_softmax(student_cls_c[v], dim=-1), dim=-1)
    ...
    def update_center_accum(self, teacher_cls, teacher_patch):
        ...
        with jt.no_grad():
            ...
            patch_center = jt.sum(teacher_patch.mean(1, keepdim=True), dim=0, keepdim=True)
            if jt.in_mpi:
                patch_center = patch_center.mpi_all_reduce('add')
            self.center2_accum = self.center2_accum + patch_center


class DataAugmentationSERE(object):
    ...
    for iq, q in enumerate(teacher_out):
        for v in range(len(student_out)):
            if v == iq:
                ...
                continue
            loss = jt.sum(-jt.nn.softmax(q, dim=-1) * jt.nn.log_softmax(student_out[v], dim=-1), dim=-1)
    ...
    if student_output_local is not None:
        for iq, q in enumerate(teacher_out_global):
            for v in range(len(student_out_local)):
                ...
                loss = jt.sum(-jt.nn.softmax(global_crop, dim=-1) * jt.nn.log_softmax(local_crop, dim=-1), dim=-1)
                total_loss += loss.mean()
                n_loss_terms += 1
...
def sere_loss_channel(args, student_output, teacher_output, student_output_local, ncrops, ncrops_local):
    ...
    teacher_out = jt.nn.softmax(teacher_out, dim=-1).detach().chunk(ncrops)
    ...
    for iq, q in enumerate(teacher_out):
        for v in range(len(student_out)):
            if v == iq:
                # we skip cases where student and teacher operate on the same view
                continue
            loss = jt.sum(-q * jt.nn.log_softmax(student_out[v], dim=-1), dim=-1)
            total_loss += loss.mean()
            n_loss_terms += 1

        if student_output_local is not None:
            for v in range(len(student_out_local)):
                loss = jt.sum(-q * jt.nn.log_softmax(student_out_local[v], dim=-1), dim=-1)
                total_loss += loss.mean()
                n_loss_terms += 1
